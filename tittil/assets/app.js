// ── Tittil — Tiba Tiba Lapar (optimized) ──
const RESTAURANT_SLUG = 'tittil', API_BASE = '/api', CART_KEY = 'cart_' + RESTAURANT_SLUG;
const KHR_RATE = 4000;
let cart = [], allItems = [], activeCat = 'all';
...
function renderCartList() {
  ...
  cart.forEach(i => {
    ...
    html += `<div class="flex items-start gap-3 py-3.5 border-b border-black/5">
      <div class="flex-1 min-w-0">
        <div class="text-sm font-medium text-[#1A1A1A] leading-snug truncate">${esc(i.name)}</div>
        <div class="flex items-baseline gap-2 mt-0.5">
          <span class="text-xs text-[#8E8E93]">${fmt(i.price)}</span>
          <span class="text-[10px] text-[#B0B0B0]">${fmtKHR(i.price)}</span>
        </div>
      </div>
      <div class="flex items-center gap-1.5 shrink-0">
        <button onclick="changeQty('${i.id}',-1)" class="w-7 h-7 rounded-full bg-[#F5F0EB] border border-black/5 text-xs font-bold text-[#8E8E93] hover:bg-[#FFC700] hover:text-black hover:border-[#FFC700] transition-all">−</button>
        <span class="w-7 text-center text-sm font-semibold text-[#1A1A1A]">${i.qty}</span>
        <button onclick="changeQty('${i.id}',1)" class="w-7 h-7 rounded-full bg-[#F5F0EB] border border-black/5 text-xs font-bold text-[#8E8E93] hover:bg-[#FFC700] hover:text-black hover:border-[#FFC700] transition-all">+</button>
      </div>
      <div class="text-right shrink-0 min-w-[65px]">
        <div class="text-sm font-bold text-[#C49000]">${ln}</div>
        <div class="text-[10px] text-[#B0B0B0]">${lnKHR}</div>
      </div>
      <button onclick="removeItem('${i.id}')" class="shrink-0 w-6 h-6 flex items-center justify-center rounded-full text-[#B0B0B0] hover:text-red-400 hover:bg-red-400/10 transition-all text-xs">×</button>
    </div>`;
  });
  body.innerHTML = html;
  if (!footer) return;
  ...
  footer.innerHTML = `<div class="flex justify-between items-baseline pt-2 pb-1">
    <span class="text-xs text-[#8E8E93]">${n} item${n>1?'s':''}</span>
    <span class="text-xs text-[#8E8E93]">Subtotal</span>
  </div>
  <div class="flex justify-between items-baseline pb-3 border-b border-black/5">
    <span class="text-xs text-[#B0B0B0]">${totKHR}</span>
    <span class="text-base font-bold text-[#1A1A1A]">${fmt(tot)}</span>
  </div>
  <div class="space-y-2.5 pt-3">
    <input id="cust-name" type="text" placeholder="Your name" class="w-full px-3.5 py-2.5 rounded-lg bg-white border border-black/10 text-sm placeholder-[#B0B0B0] focus:outline-none focus:border-[#FFC700] focus:ring-1 focus:ring-[#FFC700]/20 transition-all">
    <input id="cust-phone" type="tel" placeholder="Phone number" class="w-full px-3.5 py-2.5 rounded-lg bg-white border border-black/10 text-sm placeholder-[#B0B0B0] focus:outline-none focus:border-[#FFC700] focus:ring-1 focus:ring-[#FFC700]/20 transition-all">
    <textarea id="cust-notes" rows="2" placeholder="Order notes (optional)" class="w-full px-3.5 py-2.5 rounded-lg bg-white border border-black/10 text-sm placeholder-[#B0B0B0] resize-none focus:outline-none focus:border-[#FFC700] focus:ring-1 focus:ring-[#FFC700]/20 transition-all"></textarea>
  </div>
  <div class="pt-3">
    <div class="text-[11px] font-semibold text-[#8E8E93] uppercase tracking-wider mb-2">Payment Method</div>
    <div class="flex gap-3">
      <label class="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-lg border-2 cursor-pointer transition-all" id="pay-cash" style="border-color:#4CAF50;background:rgba(76,175,80,0.06)">
        <input type="radio" name="payment" value="cash" checked class="accent-green-600" onchange="selectPayment('cash')">
      ...
      <label class="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-lg border-2 cursor-pointer transition-all" id="pay-aba" style="border-color:rgba(239,83,80,0.3);background:rgba(239,83,80,0.04)">
        <input type="radio" name="payment" value="aba" class="accent-red-600" onchange="selectPayment('aba')">
        <span class="text-sm font-medium">💳 ABA Transfer</span>
      </label>
    </div>
  </div>
  <button onclick="placeOrder()" class="w-full bg-[#FFC700] text-black font-bold py-3.5 rounded-full mt-4 flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-lg shadow-[#FFC700]/10 text-sm tracking-wide">
...
async function placeOrder() {
  ...
  try {
    ...
    if (data.success) {
      ...
      const cn = document.getElementById('confirm-num'); if (cn) cn.textContent = '#' + data.data.orderNumber;
      ...
      if (cb) {
        ...
        billItems.forEach(i => {
          billHTML += `<div class="flex justify-between items-center py-1.5 text-xs">
            <span class="text-[#1A1A1A] text-left flex-1">${esc(i.name)} × ${i.qty}</span>
            <span class="text-[#8E8E93] ml-2">${fmt(i.price * i.qty)}</span>
          </div>`;
        });
        billHTML += `<div class="flex justify-between items-center pt-2 mt-2 border-t border-black/5 text-xs">
          <span class="text-[#B0B0B0]">Payment</span>
          <span class="text-[#1A1A1A]">${billPayment === 'aba' ? '💳 ABA Transfer' : '💵 Cash'}</span>
        </div>`;
        billHTML += `<div class="flex justify-between items-center pt-2 mt-1 border-t border-black/5">
          <span class="text-sm font-bold text-[#1A1A1A]">Total</span>
          <span class="text-sm font-bold text-[#C49000]">${fmt(billTotal)}</span>
        </div>`;
        cb.innerHTML = billHTML;
      }
      const oc = document.getElementById('order-confirm'); if (oc) oc.classList.remove('hidden');
      const tgd = document.getElementById('tg-delay');
      if (tgd) { tgd.style.display = 'none'; setTimeout(() => { tgd.classList.remove('hidden'); tgd.classList.add('flex','tg-pulse'); toast('Chat via Telegram sekarang!'); }, 300000); }
...
async function loadMenu() {
  try {
    ...
    if (el) el.innerHTML = '<p class="text-center py-8 text-[#B0B0B0]">Failed to load menu.</p>';
  }
}
...
function renderMenu(items) {
  const el = document.getElementById('menu-list');
  if (!el) return;
  if (!items.length) { el.innerHTML = '<p class="text-center py-8 text-[#B0B0B0]">No menu available.</p>'; return; }
  ...
  Object.entries(cats).forEach(([cat, its]) => {
    ...
    hdr.innerHTML = `<h2 class="font-['Playfair_Display',serif] text-lg font-bold text-[#1A1A1A]">${esc(cat)}</h2>`;
    frag.appendChild(hdr);

    its.forEach(i => {
      const row = document.createElement('div');
      row.className = 'menu-row flex items-center justify-between py-4 px-4 bg-white rounded-xl mb-2 cursor-pointer active:scale-[0.98] transition-transform shadow-sm border border-black/[0.04]';
      ...
      row.innerHTML = `<div class="flex-1 min-w-0 pr-3">
        <div class="font-medium text-sm text-[#1A1A1A]">${esc(i.name)}</div>
        ${i.description ? `<div class="text-xs text-[#8E8E93] truncate">${esc(i.description)}</div>` : ''}</div>
        <div class="flex items-center gap-3 shrink-0">
        <div class="text-right"><div class="font-bold text-sm text-[#C49000]">${fmt(i.price)}</div>
        <div class="text-[10px] text-[#B0B0B0]">${fmtKHR(i.price)}</div></div>
        <button class="w-9 h-9 rounded-full bg-[#FFC700] flex items-center justify-center shadow-sm active:scale-90 transition-transform hover:scale-110">+
        </button></div>`;
      const btn = row.querySelector('button');
      if (btn) btn.onclick = function(e) { e.stopPropagation(); addItem(i.id, i.name, i.price, i.stockQuantity ?? 999); };
...
// ── Food Image Drawer ──
const FOOD_IMG_BASE = '/tittil/tittil/';
const FOOD_FILES = [
  'Adaan-Bulat_5.50_dolar.jpg','CUKO_1_dolar.jpg [Additional Menu].jpg','Celimpungan_5.50_dolar.jpg',
  'Es-Jeruk-Asli_2.50_dolar.jpg [Minuman].jpg','Kapal-Selam-Telur-Bebek_6_dolar.jpg','Kapal-Selam_5.50_dolar.jpg',
  'Keriting-Goreng_5.50_dolar.jpg','Keriting-Rebus_5.50_dolar.jpg','Kukubima-Drink_1.50_dolar.jpg [Minuman].jpg',
  'Lenggang-Goreng-Telur-Ayam_4.50_dolar.jpg','Lenggang-Goreng-Telur-Bebek_5.50_dolar.jpg','Lenjer-Balok_5.50_dolar.jpg',
  'Model-Telur-Tekwan_6_dolar.jpg','Model-Telur_5.50_dolar.jpg','Nasi-Pindang-Ikan-Patin_5.50_dolar.jpg',
  'Nasi-Putih_1_dolar.jpg [Additional Menu].jpg','Pecel-Ayam_4.50_dolar.jpg [Rice Dishes].jpg','Pecel-Lele_4.50_dolar.jpg [Rice Dishes].jpg',
  'Pempek-Mix-Komplit_12_dolar.jpg','Pempek-Mix_9.50_dolar.jpg','Pempek-Tahu_5.50_dolar.jpg','Pistel_5.50_dolar.jpg',
  'Rujak-Mie_4.50_dolar.jpg','Soto-Lamongan-Nasi_4.50_dolar.jpg','Soto-Tangkar-Nasi_6_dolar.jpg','Tekwan_4.50_dolar.jpg',
  'Telur-Kecil_5.50_dolar.jpg','soto-daging.jpg'
];
const FOOD_MAP = Object.fromEntries(FOOD_FILES.map(f => [
  f.replace(/_\d+\.\d+_dolar.*?\.jpg/i,'').replace(/\s*\[.*?\]\s*/g,' ').replace(/[-_]/g,' ').replace(/\s+/g,' ').trim().toLowerCase(), f
]));
...
function showFoodDrawer(name, price, cat) {
  ...
  const d = document.getElementById('food-drawer'); if (!d) return;
  d.classList.remove('invisible');
...
function getChatName() {
  ...
  if (n) { localStorage.setItem('tittil_chat_name', n); return n; }
  return localStorage.getItem('tittil_chat_name') || ('Guest_' + Math.random().toString(36).substr(2,4));
}
...
function addChatMsg(m, side) {
  ...
  d.innerHTML = `<div class="${side==='right'?'bg-[#FFC700] text-black rounded-2xl rounded-br-sm':'bg-[#F5F0EB] rounded-2xl rounded-bl-sm'} px-3 py-2 text-sm max-w-[80%]">${esc(m)}</div>`;
