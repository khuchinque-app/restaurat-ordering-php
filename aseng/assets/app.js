...
// Collect the customer name from wherever they enter it
function getChatCustomerName() {
    const nameInput = document.getElementById('cust-name');
    if (nameInput && nameInput.value.trim()) {
        chatName = nameInput.value.trim();
        localStorage.setItem('aseng_chat_name', chatName);
        return chatName;
    }
    chatName = localStorage.getItem('aseng_chat_name');
    if (!chatName) {
...
async function sendChatMessage() {
    ...
    try {
        ...
        if (data.success) {
            ...
            sessionStorage.setItem('aseng_chat_id', data.data.id);
        }
    } catch {}
}
...
async function loadChatHistory() {
    try {
        // Use the stored customer name to look up this conversation
